...
public  class JoyhentaiDownload extends Reptile{
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			try {
				...
				downloadPath = "C:\\迅雷下载\\和谐物\\2108\\qq";
			}
			if("exit".equals(url))
				break;
			...
			Thread thread = new Thread(new Runnable() {
				@Override
				public void run() {
					JoyhentaiDownload erocoolDownload = new JoyhentaiDownload();
					try {
						erocoolDownload.download(url, finalDownloadPath);
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
	...
	public void download(String url, String target) throws Exception {
		...
		Elements imgDiv = document.select(".bookview-wrap>div>div>img");
